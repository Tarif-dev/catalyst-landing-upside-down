...
async function testFlow() {
  ...
  await supabase.from("profiles").update({
    full_name: "Dustin Henderson", phone: "111-111-1111", college: "Hawkins High"
  ...
  const { data: aMembers } = await supabase.from("team_members").select("*").eq("team_id", team.id);
  console.log(`   Team size after creation: ${aMembers.length} (Expected 1)`);
  if (aMembers.length !== 1 || aMembers[0].role !== 'leader') throw new Error("Leader not in team_members");
  ...
  await supabase.from("profiles").update({
    full_name: "Mike Wheeler", phone: "222-222-2222", college: "Hawkins High"
  }).eq("user_id", userBData.user.id);
  console.log("   User B registered.");

  // 4. User B joins team via code
  console.log("\n4. User B joining team via code...");
  const { data: joinRes, error: joinErr } = await supabase.rpc("join_team_by_code", { p_code: team.pass_code });
  if (joinErr) throw joinErr;
  console.log(`   User B successfully joined team ${joinRes}`);

  // Verify Team Size = 2
  const { data: bMembers } = await supabase.from("team_members").select("*").eq("team_id", team.id);
  ...
  const { error: bTrackErr } = await supabase.rpc("change_team_track", { p_new_track: "fintech" });
  console.log(bTrackErr ? `   Correctly caught error: ${bTrackErr.message}` : "   FAILED: User B was able to change track!");
  ...
  // Verify Team Size = 1
  const { data: cMembers } = await supabase.from("team_members").select("*").eq("team_id", team.id);
  ...
  const { error: aTrackErr } = await supabase.rpc("change_team_track", { p_new_track: "healthcare" });
  if (aTrackErr) throw aTrackErr;
  
  const { data: teamAfterTrack } = await supabase.from("teams").select("track").eq("id", team.id).single();
  ...
  const { error: leaderErr } = await supabase.rpc("change_team_leader", { p_new_leader_id: userBData.user.id });
  if (leaderErr) throw leaderErr;
  console.log("   Leadership transferred.");

  // Verify Role in team_members
  const { data: finalMembers } = await supabase.from("team_members").select("role").eq("user_id", userAData.user.id).single();
