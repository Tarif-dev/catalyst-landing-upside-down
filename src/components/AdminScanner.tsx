...
export function AdminScanner({ accessToken }: { accessToken: string }) {
  ...
  const [quickAction, setQuickAction] = useState<keyof Actions | "none">(
    "none",
  );
  ...
  const addHistory = useCallback(
    (name: string, passCode: string, action: string, success: boolean) => {
    ...
    async (code: string) => {
      ...
      try {
        const res = await lookupFn({
          data: { accessToken, passCode: trimmed },
        });
        if (!res.found) {
          ...
          triggerFlash("error");
          addHistory(
            (res.participant as any)?.name || "Unknown",
            trimmed,
            "Not Verified",
            false,
          );
          setResult({
            participant: res.participant as ParticipantInfo,
            actions: {
              gate_entry: false,
              checked_in: false,
              meal_1: false,
          ...
          addHistory(
            (res.participant as ParticipantInfo).name,
            trimmed,
            "Lookup ✓",
            true,
          );
            ...
            setTimeout(
              () =>
                autoFireAction(
                  trimmed,
                  quickAction,
                  res.participant as ParticipantInfo,
                  res.actions!,
                ),
              300,
            );
    ...
    try {
      ...
      if (res.alreadyUsed) {
        ...
        addHistory(
          participant.name,
          passCode,
          `${ACTION_CONFIG.find((a) => a.key === action)?.label} (already)`,
          false,
        );
      } else {
        ...
        addHistory(
          participant.name,
          passCode,
          ACTION_CONFIG.find((a) => a.key === action)?.label || action,
          true,
        );
      }
  ...
  const handleAction = async (action: keyof Actions) => {
    ...
    try {
      const res = await recordFn({
        data: { accessToken, passCode: result.participant.passCode, action },
      });
      if (res.alreadyUsed) {
        toast.warning(res.message);
        playTone(false);
        ...
        addHistory(
          ...
          `${ACTION_CONFIG.find((a) => a.key === action)?.label} (dup)`,
          false,
        );
      } else {
        toast.success(res.message);
  ...
  const handleUndo = async (action: keyof Actions) => {
    ...
    try {
      ...
      addHistory(
        result.participant.name,
        result.participant.passCode,
        `Undo ${ACTION_CONFIG.find((a) => a.key === action)?.label}`,
        true,
      );
      setResult((prev) => (prev ? { ...prev, actions: res.actions } : prev));
    } catch (err: any) {
  ...
  const handleReset = async () => {
    ...
    try {
      const res = await resetFn({
        data: { accessToken, passCode: result.participant.passCode },
      });
      toast.success(res.message);
      addHistory(
        result.participant.name,
      ...
      >
        ...
        {quickAction !== "none" && (
          <>
            ...
            >
              ⚡ Quick:{" "}
              {ACTION_CONFIG.find((a) => a.key === quickAction)?.label}
            </span>
          </>
        )}
      </div>
          ...
          {mode === "scan" ? (
            <div>
              ...
              {quickAction !== "none" && (
                ...
                >
                  ⚡ Auto-firing "
                  {ACTION_CONFIG.find((a) => a.key === quickAction)?.label}" on
                  successful scan
                </p>
              )}
            </div>
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    opacity: looking || !manualCode.trim() ? 0.5 : 1,
                  }}
                >
                  {looking ? "..." : "Lookup"}
                </button>
        ...
        <div style={{ display: "grid", gap: 16 }}>
          ...
          <div
            style={{
              ...
              border: `2px solid ${result.verified ? "#86efac" : "#fecaca"}`,
              borderRadius: 12,
              padding: 20,
              transition: "border-color 0.3s",
            }}
            ...
            >
                ...
                <div style={{ marginTop: 8, display: "grid", gap: 3 }}>
                  {result.participant.teamName && (
                    <p style={{ margin: 0, fontSize: 14, color: "#374151" }}>
                      <strong>Team:</strong> {result.participant.teamName}
                      {result.participant.role === "leader" ? " (Leader)" : ""}
                    </p>
                  )}
                  {result.participant.track && (
                    <p style={{ margin: 0, fontSize: 14, color: "#374151" }}>
                      <strong>Track:</strong>{" "}
                      {result.participant.track.charAt(0).toUpperCase() +
                        result.participant.track.slice(1)}
                    </p>
                  )}
              ...
              <div
                style={{
                  ...
                  fontSize: 28,
                  fontWeight: 800,
                  color: result.verified ? "#059669" : "#b91c1c",
                  letterSpacing: "0.1em",
                }}
              >
                {result.participant.passCode}
              ...
              {ACTION_CONFIG.map(({ key, label, icon, color, bg }) => (
                <div
                  ...
                  style={{
                    ...
                    padding: "8px 4px",
                    borderRadius: 8,
                    background: result.actions[key] ? bg : "#f9fafb",
                    border: `1px solid ${result.actions[key] ? color : "#e5e7eb"}40`,
                    transition: "all 0.2s",
                  }}
                >
                  <div style={{ fontSize: 18 }}>
                    {result.actions[key] ? "✓" : icon}
                  </div>
                  <div
                    style={{
                      fontSize: 10,
                      fontWeight: 700,
                      color: result.actions[key] ? color : "#9ca3af",
                      marginTop: 2,
                    }}
                  >
                    {label}
            ...
            >
              ...
              >
                {ACTION_CONFIG.map(({ key, label, icon, color }) => {
                  ...
                  const isBusy =
                    actionBusy === key || actionBusy === `undo-${key}`;
                  return (
                    <div
                      key={key}
                      style={{
                        border: `2px solid ${used ? "#e5e7eb" : color}`,
                        borderRadius: 10,
                        overflow: "hidden",
                        transition: "all 0.15s",
                      }}
                    >
                      <button
                        onClick={() =>
                          used ? handleUndo(key) : handleAction(key)
                        }
                        disabled={isBusy}
                        style={{
                          width: "100%",
                          padding: "16px 12px",
                          border: "none",
                          background: used ? "#f9fafb" : color,
                          color: used ? "#6b7280" : "#fff",
              ...
              >
                <button
                  ...
                  disabled={
                    actionBusy === "reset" ||
                    (!result.actions.gate_entry &&
                      !result.actions.checked_in &&
                      !result.actions.meal_1 &&
                      !result.actions.meal_2)
                  }
                  style={{
                    padding: "6px 16px",
                    borderRadius: 6,
